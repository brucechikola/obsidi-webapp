...
const NavigationLink = ({title="", url="", className="", width=130}) =>{
    return <a href={url} className={`${className} !h-[50px] h-full flex items-center justify-center btn transition duration-100 rounded-xl`}>{title}</a>
}
    ...
    <div className="w-full h-[70px] border-b border-b-slate-300 relative">
        <div className="w-[90%] mx-auto h-full flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center justify-end gap-x-5 text-[14px]">
                <NavigationLink title="Sign In" className=" w-[90px] hover:bg-slate-100" />
                <NavigationLink title="Register" className=" w-[90px] hover:bg-slate-100" />
